...
absolute_time_t start_time; // Zeit Anfang der Messung
absolute_time_t end_time;  // Zeit Ende der Messung
...
int main() {
    ...
    // ADC initialisieren
    adc_init();
    adc_gpio_init(HALL_SENSOR_PIN); // GPIO26 als ADC-Eingang konfigurieren (Kanal 0)
    adc_select_input(0);
    ...
    gpio_set_irq_enabled_with_callback(SWITCH_PIN, GPIO_IRQ_EDGE_FALL, true, &switch_isr); // Interrupt Switch, Edge Fall da Pull-Up Widerstand
